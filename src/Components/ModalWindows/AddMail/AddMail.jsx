...
export default function AddMail({
  ...
  const [responsibleUserIds, setResponsibleUserIds] =
    useState([]);
  const [responsibleQuery, setResponsibleQuery] =
    useState("");
  const [responsibleOpened, setResponsibleOpened] =
    useState(false);
    ...
    <AddMailView
      ...
      handleShowPassword={async () => {
        ...
        try {
          ...
          console.error(
            "Помилка при завантаженні пароля:",
            error
          );

          setPasswordError(
            error?.response?.data?.message ||
            "Не вдалося завантажити пароль."
          );
        } finally {
